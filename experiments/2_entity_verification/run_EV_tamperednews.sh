...
questionFile=./experiments/2_entity_verification/_questions/questions_tamperednews.jsonl
...
if [ $1 -eq 1 ]
then
    python ./experiments/2_entity_verification/scripts/tamperednews/prepare_questions.py --question-file $questionFile &
    PID=$!
    wait $PID
fi
...
answerFile=EV-tamperednews-blip_2_answers
...
answerFile=EV-tamperednews-instructBlip_answers
activeModels+=(${answerFile})
if [ $2 -eq 1 ]
then
...
answerFile=EV-tamperednews-llava_1_5_7b_answers
activeModels+=(${answerFile})
if [ $2 -eq 1 ]
then
...
answerFile=EV-tamperednews-llava_1_5_13b_answers
activeModels+=(${answerFile})
if [ $2 -eq 1 ]
then
...
answerFile=EV-tamperednews-llava_1_6_7b_answers
activeModels+=(${answerFile})
if [ $2 -eq 1 ]
then
    python ./model_scripts/llava.py --question-file $questionFile --answer-file-path $answerFilePath --model-path $modelPath --answer-file-name $answerFile &
...
python ./experiments/2_entity_verification/scripts/tamperednews/analyze_answers.py --models ${activeModels[@]} &
PID=$!
wait $PID

python ./experiments/2_entity_verification/scripts/tamperednews/printResultTable.py --models ${activeModels[@]} &
PID=$!
wait $PID