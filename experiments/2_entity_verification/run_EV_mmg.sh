...
questionFile=./experiments/2_entity_verification/_questions/questions_mmg.jsonl
...
if [ $1 -eq 1 ]
then
    python ./experiments/2_entity_verification/scripts/mmg/prepare_questions.py --question-file $questionFile &
    PID=$!
    wait $PID
fi
...
answerFile=EV-mmg-blip_2_answers
...
answerFile=EV-mmg-instructBlip_answers
activeModels+=(${answerFile})
if [ $2 -eq 1 ]
then
...
answerFile=EV-mmg-llava_1_5_7b_answers
activeModels+=(${answerFile})
if [ $2 -eq 1 ]
then
...
answerFile=EV-mmg-llava_1_5_13b_answers
activeModels+=(${answerFile})
if [ $2 -eq 1 ]
then
...
answerFile=EV-mmg-llava_1_6_7b_answers
activeModels+=(${answerFile})
if [ $2 -eq 1 ]
then
    python ./model_scripts/llava.py --question-file $questionFile --answer-file-path $answerFilePath --model-path $modelPath --answer-file-name $answerFile &
...
python ./experiments/2_entity_verification/scripts/mmg/analyze_answers.py --models ${activeModels[@]} &
PID=$!
wait $PID

python ./experiments/2_entity_verification/scripts/mmg/printResultTable.py --models ${activeModels[@]} &
PID=$!
wait $PID