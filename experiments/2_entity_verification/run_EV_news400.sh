...
questionFile=./experiments/2_entity_verification/_questions/questions_news400.jsonl
...
if [ $1 -eq 1 ]
then
    python ./experiments/2_entity_verification/scripts/news400/prepare_questions.py --question-file $questionFile &
    PID=$!
    wait $PID
fi
...
answerFile=EV-news400-blip_2_answers
...
answerFile=EV-news400-instructBlip_answers
activeModels+=(${answerFile})
if [ $2 -eq 1 ]
then
...
answerFile=EV-news400-llava_1_5_7b_answers
activeModels+=(${answerFile})
if [ $2 -eq 1 ]
then
...
answerFile=EV-news400-llava_1_5_13b_answers
activeModels+=(${answerFile})
if [ $2 -eq 1 ]
then
...
answerFile=EV-news400-llava_1_6_7b_answers
activeModels+=(${answerFile})
if [ $2 -eq 1 ]
then
    python ./model_scripts/llava.py --question-file $questionFile --answer-file-path $answerFilePath --model-path $modelPath --answer-file-name $answerFile &
...
python ./experiments/2_entity_verification/scripts/news400/analyze_answers.py --models ${activeModels[@]} &
PID=$!
wait $PID

python ./experiments/2_entity_verification/scripts/news400/printResultTable.py --models ${activeModels[@]} &
PID=$!
wait $PID