...
answerFile=11_DV-mmg-instructBlip
activeModels+=(${answerFile})
if [ $3 -eq 1 ]
then
...
answerFile=11_DV-mmg-llava_15_7b
activeModels+=(${answerFile})
if [ $3 -eq 1 ]
then
...
answerFile=11_DV-mmg-llava_15_13b
activeModels+=(${answerFile})
if [ $3 -eq 1 ]
then
...
answerFile=11_DV-mmg-llava_16_7b
activeModels+=(${answerFile})
if [ $3 -eq 1 ]
then
    python ./model_scripts/llava.py --question-file $questionFile --answer-file-path $answerFilePath --model-path $modelPath --answer-file-name $answerFile &
