...
def generate1x1Image(newsImage, entityImage, questionID, entityID, counter):
    ...
    if w_1 > h_1:
        ...
        combinedImage.save(f"./4_multi_image_entity_verification/images/tamperednews/{questionID}_{entityID}-{counter}.jpg", quality=95)    
...
def saveQuestion(args, id, entityID, counter, question, entity, testlabel, set, ground_truth, ground_wrong):
    with open(args.question_file, "a") as outfile:
        outfile.write("""{\"question_id\": \"%s_%s\", \"image\": \"./4_multi_image_entity_verification/images/tamperednews/%s_%s-%s.jpg\", \"question\": %s, \"entity\": \"%s\", \"testlabel\": \"%s\", \"set\": \"%s\", \"gTruth\": \"%s\", \"gWrong\": \"%s\"} \n""" 
