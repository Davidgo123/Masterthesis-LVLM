...
def generate1x1Image(newsImage, entityImage, questionID, entityID):
    ...
    if w_1 > h_1:
        ...
        combinedImage.save(f"./experiments/3_image_entity_verification/images/news400/{questionID}_{entityID}.jpg", quality=95)    
...
def saveQuestion(args, id, entityID, question, entity, testlabel, set, ground_truth, ground_wrong):
    with open(args.question_file, "a") as outfile:
        outfile.write("""{\"question_id\": \"%s\", \"image\": \"./experiments/3_image_entity_verification/images/news400/%s_%s.jpg\", \"question\": %s, \"entity\": \"%s\", \"testlabel\": \"%s\", \"set\": \"%s\", \"gTruth\": \"%s\", \"gWrong\": \"%s\"} \n""" 
