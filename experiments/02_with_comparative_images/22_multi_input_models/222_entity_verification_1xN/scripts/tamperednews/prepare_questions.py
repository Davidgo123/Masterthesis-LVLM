                            ...
                            for counter, entity_image in enumerate(entityFiles):
                                ...
                                if 'visible' in lineObject['test_' + entityObject['name']]:
                                    if entityID in lineObject['test_' + entityObject['name']]['visible']:
                                        saveQuestion(args, str(lineObject['id']), str(news_image), str(entity_image), str(counter), str(question), str(entityObject['name']), "orginal", "text", "yes", "no")
                                    else:
...
def saveQuestion(args, id, news_image, entity_image, question, entity, testlabel, set, ground_truth, ground_wrong):
    with open(args.question_file, "a") as outfile:
        outfile.write("""{\"question_id\": \"%s_%s\", \"news_image\": \"%s\", \"entity_image\": \"%s\", \"question\": %s, \"entity\": \"%s\", \"testlabel\": \"%s\", \"set\": \"%s\", \"gTruth\": \"%s\", \"gWrong\": \"%s\"} \n""" 
