                        ...
                        for entityID in lineObject['test_' + entityObject['name']]['untampered']:
                            ...
                            if 'visible' in lineObject['test_' + entityObject['name']]:
                                if entityID in lineObject['test_' + entityObject['name']]['visible']:
                                    saveQuestion(args, str(lineObject['id']), str(news_image), str(entity_image), str(question), str(entityObject['name']), "orginal", "text", "yes", "no")
                                else:
                                    saveQuestion(args, str(lineObject['id']), str(news_image), str(entity_image), str(question), str(entityObject['name']), "orginal", "text", "no", "yes")
                            else:
                                saveQuestion(args, str(lineObject['id']), str(news_image), str(entity_image), str(question), str(entityObject['name']), "orginal", "text", "no", "yes")

# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    
def saveQuestion(args, id, news_image, entity_image, question, entity, testlabel, set, ground_truth, ground_wrong):
    with open(args.question_file, "a") as outfile:
        outfile.write("""{\"question_id\": \"%s\", \"news_image\": \"%s\", , \"entity_image\": \"%s\", \"question\": %s, \"entity\": \"%s\", \"testlabel\": \"%s\", \"set\": \"%s\", \"gTruth\": \"%s\", \"gWrong\": \"%s\"} \n""" 
                      % (id, news_image, entity_image, question, entity, testlabel, set, ground_truth, ground_wrong))
