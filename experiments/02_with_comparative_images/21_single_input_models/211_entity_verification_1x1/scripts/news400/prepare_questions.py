...
def generate1x1Image(args, newsImage, entityImage, questionID, entityID):
    ...
    if w_1 > h_1:
        ...
        combinedImage.save(f"{args.base_path}/images/news400/{questionID}_{entityID}.jpg", quality=95)    
...
def saveQuestion(args, id, entityID, question, entity, testlabel, set, ground_truth, ground_wrong):
    with open(args.question_file, "a") as outfile:
        outfile.write("""{\"question_id\": \"%s\", \"image\": \"%s/images/news400/%s_%s.jpg\", \"question\": %s, \"entity\": \"%s\", \"testlabel\": \"%s\", \"set\": \"%s\", \"entityID\": \"%s\", \"gTruth\": \"%s\", \"gWrong\": \"%s\"} \n""" 
