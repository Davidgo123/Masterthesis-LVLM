...
class llavaInstance:
    def __init__(self, args):
        ...
        self.model = LlavaNextForConditionalGeneration.from_pretrained(args.model_path, torch_dtype=torch.float16)
    ...
    def getResponse(self, args, prompt, image):
        inputs = self.processor(prompt, image, return_tensors="pt").to(args.device, torch.float16)
        with torch.no_grad():
...
def run(args, answerFile):
    ...
    with open(args.question_file, encoding="utf-8", mode='r') as file:
        ...
        for question in questions:
            prompt = f"[INST] <image> {question['question']} [/INST]"
            ...
            response = response[str(response).find("[/INST] ") + len("[/INST] "):]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-path", type=str, default="./models/llava-1.5-7b-hf/")
